...
function getKeys() {
  ...
  if (!keyId || !keySecret) {
    // Name the right place to fix it. Hosted environments never read a local .env,
    // so pointing at that file sends you looking in the wrong place.
    const missing = [!keyId && 'RAZORPAY_KEY_ID', !keySecret && 'RAZORPAY_KEY_SECRET']
      .filter(Boolean)
      .join(' and ');
    let where = 'your .env file';
    if (process.env.NETLIFY) {
      where = "Netlify's dashboard (Site configuration → Environment variables), then redeploy";
    } else if (process.env.VERCEL) {
      where = "Vercel's dashboard (Settings → Environment variables), then redeploy";
    }
    const err = new Error('Razorpay credentials missing. Set ' + missing + ' in ' + where + '.');
    err.statusCode = 500;
    throw err;
  }
